...
typedef struct _auth_serv_t {
	...
	//读取wifidog的配置文件的时候会设置...也有默认的path=/wifidog/
...
typedef struct {
    ...
    //客户端的超时时间...也就是客户端连接超过的时间后需要重新认证啦...
    //单位是checkinterval的数值...how many checkintarvals..
    int clienttimeout;          /**< @brief How many CheckIntervals before a client
				     must be re-authenticated */
    //ping线程的运行频率...用来设置它睡眠的时间...
	...
	//规则的集合...rulesets是一个集合的组成,每一个集合有自己的名称..保存在name字段..
	//rules字段指向这个集合里面的规则..(多条规则组成一个链表.)
    t_firewall_ruleset *rulesets;       /**< @brief firewall rules */
    t_trusted_mac *trustedmaclist; /**< @brief list of trusted macs */
    char *arp_table_path; /**< @brief Path to custom ARP table, formatted
        like /proc/net/arp */
    //在连接到认证服务器列表时候...可能无法解析服务器的域名...此时为了判断是不是因为DNS的原因...
    //使用这里通用(popular)的服务器...因此popular_servers存放的就是一些通用的服务器...比如www.google.com www.baidu.com 等..
