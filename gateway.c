...
get_clients_from_parent(void)
{
    ...
    sa_un.sun_family = AF_UNIX;
						// 默认internal_sock "/tmp/wifidog.sock"
...
*/                       
    while (read(sock, &onechar, 1) == 1) {
        ...
        if (!onechar) {
            ...
            running1 = linebuffer;
	//strsep是用来分割字符串...
	//假设linebuffer = "hello|world|yes|no"; token1 = strsep(&linebuffer, "|");
	//返回之后会使第一个"|"替换为'\0'，就是这样分割了字符串...
	//token1指向"hello"  linebuffer指向"world|yes|no"
            while ((token1 = strsep(&running1, "|")) != NULL) {
	//下面的操作过程是先获取command..因为传递过来的字符串其实是有规律的。。
	//command|key=value|key=value|key=value.
	//每执行一次"|"分隔就是获取一个key=value的过程...
			...
			//处理完了一个命令之后...也就是获取到了一个客户之后client.
            if (client) {
				//插入到client的链表当中去...
...
//网关IP地址和接口....没有设置address的时候..通过interface可以得到gw的address 
    ...
    while (1) {
		...
		//接受一个http连接请求...这里timeout设置为NULL..估计就是阻塞等待http的连接了..
