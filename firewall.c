...
fw_init(void)
{
    ...
    if (restart_orig_pid) {
		//有可能执行的是因为restart参数的..
		//要重新启动..但是执行过程中，可能已经有客户连接当中,也就是有客户已经是通过认证了的。。
		//那就算restart之后，这些客户也应该不要被限制..fw_allow就是这个作用.
...
fw_sync_with_authserver(void)
{
    ...
    for (p1 = p2 = worklist; NULL != p1; p1 = p2) {
        ...
        if (p1->counters.last_updated + (config->checkinterval * config->clienttimeout) <= current_time) {
            ...
            //因为上面的操作是使用dup的方式拷贝了一个work list..然后释放锁..
            //有可能在这过程中，客户就已经退出了.因此会发现查找client list为null的情况.
