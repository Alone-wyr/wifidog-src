...
#endif
#ifndef USE_CYASSL
	//把buf数据发送到sockfd指定auth server!..并且读取返回到res中
    res = http_get(sockfd, buf);
#endif
    if (NULL == res) {
...
_connect_auth_server(int level)
{
    ...
    if (level > num_servers) {
         ...
         */
         /*
         调用自身多次...那就是递归的用法...num_servers代表当前服务器列表的数目...level每次递归的时候都会
         递增...正常来说要调用num_servers次该函数...因此level>num_servers就是超出了次数..直接返回.
         前面有level++...
         */
        return (-1);
    }
	...
	//然后会尝试去解析popular_servers...一些通用的服务器..比如www.google.. www.baidu.com ..
	//等等..它也是尝试去解析出IP地址...为的是验证是否DNS可以正常工作...
         ...
         */
        if (h_addr) {
	    //DNS是可以正常工作的...那么可能就是设置的服务器是有问题的咯.. bad.
            free(h_addr);
            /*
             * Yes
             *
        ...
        if (!auth_server->last_ip || strcmp(auth_server->last_ip, ip) != 0) {
            ...
            if (auth_server->last_ip)
                free(auth_server->last_ip);
	  //同一个域名...可以有多个ip地址???或者或，期间把ip地址换了??
            auth_server->last_ip = ip;
	  //并且要更新iptables，因此认证的服务器发生了改变....
